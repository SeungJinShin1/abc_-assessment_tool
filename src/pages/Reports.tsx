...
⭐ [추가 특별 지침: 교사용 법적 소명 및 방어 팁] ⭐
위의 1~9번에 따른 NEIS 입력용 텍스트 작성이 끝난 후, 하단에 구분선(---)을 긋고 오직 교사만을 위한 **[교사용 법적 소명 및 방어 팁]**을 추가로 작성하세요.
이 부분은 철저히 객관적이고 건조한 사실관계와 법적 논리에만 집중하며, 감정적 위로나 지지 문구는 절대 포함하지 마십시오. 이번 사안은 "학생의 상처가 특수학급에서 발생하지 않았음"을 소명하는 것이 핵심입니다.
다음 세 가지 항목을 포함하여 개조식(텍스트)으로 명확히 작성해 주세요:
- [특이사항 부재 입증]: 기록된 시간(특수학급 체류 시간) 동안 낙상, 타격 등 상처를 유발할 만한 특이사항이나 돌발행동이 없었음을 관찰 기록에 근거하여 입증하는 서술
- [보호 감독 의무 이행]: 교사가 아동복지법 등에 따른 '보호 및 감독 의무'를 다하기 위해 실행한 조치의 정당성 (책임 회피가 아닌 교사의 책무 이행 관점)
- [추가 확보 필요 증거]: 특수학급 입/퇴실 시간 교차 검증 방안, 상처 발생 시간대와 일과 시간 대조를 통해 특수학급 외에서 발생했을 가능성 입증 자료 등 객관적 알리바이 확보 방안`;
